      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Product Approval Status</h1>
              <p className="text-gray-600 text-sm sm:text-base">Track the approval status of your product submissions</p>
            </div>
            <button
              onClick={fetchProducts}
              disabled={loading}
              className="mt-4 sm:mt-0 inline-flex items-center px-4 py-3 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 touch-manipulation"
            >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
            ...
            >
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <ShoppingBag className="h-6 w-6 sm:h-8 sm:w-8 text-gray-600" />
                </div>
            ...
            >
              <div className="flex items-center">
                ...
                <div className="ml-3 sm:ml-4">
                  <p className="text-xs sm:text-sm font-medium text-gray-600">Need Revision</p>
        ...
        <div className="bg-white rounded-lg shadow-sm mb-6">
          <div className="border-b border-gray-200">
            <nav className="flex flex-wrap gap-2 sm:space-x-8 px-3 sm:px-6 py-4">
              {[
                { key: 'all', label: 'All', fullLabel: 'All Products', count: stats?.total || 0 },
                { key: 'pending', label: 'Review', fullLabel: 'Under Review', count: stats?.pending || 0 },
                { key: 'approved', label: 'Approved', fullLabel: 'Approved', count: stats?.approved || 0 },
                { key: 'rejected', label: 'Revision', fullLabel: 'Need Revision', count: stats?.rejected || 0 }
              ].map((tab) => (
                <button
                  key={tab.key}
                  onClick={() => setActiveFilter(tab.key)}
                  className={`py-2 px-3 sm:px-1 border-b-2 font-medium text-sm whitespace-nowrap touch-manipulation ${
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start space-y-3 sm:space-y-0 sm:space-x-4">
                    <img
                      ...
                      alt={product.title}
                      className="w-full h-48 sm:w-20 sm:h-20 rounded-lg object-cover"
                    />
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 mb-3">
                            ...
                            <span className="text-sm text-gray-600">
                              <span className="font-medium">Price:</span> Gh₵{product.price}
                            </span>
                            <span className="text-sm text-gray-600">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3 mt-4">
                            {product.approvalStatus === 'rejected' && (
                              <button
                                ...
                                className="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 active:bg-blue-800 transition-colors touch-manipulation"
                              >
                            ...
                            {product.approvalStatus === 'pending' && (
                              <div className="flex items-center space-x-2 text-sm text-gray-500 p-2 bg-yellow-50 rounded-lg">
                            ...
                            {product.approvalStatus === 'approved' && (
                              <div className="flex items-center space-x-2 text-sm text-green-600 p-2 bg-green-50 rounded-lg">
